...
import type {
  ...
  GameModule,
  LogEvent,
  LegacyMatchConfig as MatchConfig,
  PhaseMachine,
  Rng,
  SeatId,
} from "@benchboss/core";
...
import { LegacyObservationEnvelope as ObservationEnvelope } from "@benchboss/schemas";
...
function serveSense<State>(
  ...
  const spent = spend(
    session.budgets,
    seat,
    "budgetKey" in resolver ? resolver.budgetKey : resolver.resource,
    cost,
  );
  if (!spent.ok) {
    return {
      ...
      output: {
        ok: false,
        reason: `${"budgetKey" in resolver ? resolver.budgetKey : resolver.resource}-exhausted`,
      },
    };
  }

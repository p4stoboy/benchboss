...
const REASONS: Record<ChessOutcome["reason"], string> = {
  ...
  timeout: "time exhaustion",
  "invalid-actions": "invalid action exhaustion",
  stalemate: "stalemate",
...
export function chessResult(state: ChessState): GameResult | null {
  ...
  return {
    cause: { kind: state.outcome.cause ?? reason },
    summary:
      winner === null
...
export const plugin = {
  ...
  manifest: {
    ...
    winConditions: [
      "Checkmate the opposing king or win by the opponent's resignation or time exhaustion.",
    ...
    safeDefaults: [
      "Time exhaustion awards the opponent a win; exhausting invalid-action retries forfeits the game.",
    ],
  ...
  defaultSeats: 2,
  defaultRules: CHESS_DEFAULT_RULES,
  onHostEvent: (state: ChessState, event: HostEvent): ChessState => {
    if (state.phase === "terminal") return state;
    if (!event.seats.includes(state.players[state.position.turn]))
      throw Error("Chess expiry must identify the active player");
    return {
      ...state,
      phase: "terminal",
      pending: null,
      outcome: {
        winner: state.position.turn === "white" ? "black" : "white",
        reason: event.kind === "invalid_retries_exhausted" ? "invalid-actions" : "timeout",
        cause: event.kind,
      },
    };
  },

  ...
  return {
    ...
    legalActions(state: SpyState, seat: SeatId): LegalActionSpec[] {
      ...
      return tools.flatMap((tool) => {
        ...
        return [{ tool, phase: state.phase, jsonSchema }];
      });
    },

    submit(state: SpyState, seat: SeatId, action: SpyAction): SubmitResult<SpyState> {
      ...
      const isPhaseEnd =
        !("act" in a) && !("team" in a) && !("vote" in a) && !("sabotage" in a) && !("target" in a);
